...
$count = 0;
foreach($taby as $tab):
  $id = $tab['nazwa_zakladki'];
  $id = normalizeChars($id);
  $active = $count == 0 ? 'active' : '';
  echo '<a href="#" class="tab-links '. $active .'" data-id="'.$id.'">'.$tab['nazwa_zakladki'].'</a href="#">';
$count++;
endforeach;
echo '</div>';
...
foreach($taby as $tab):
  ...
  $id = normalizeChars($id);
  $active = $count == 0 ? 'active' : '';
  echo '<div id="'.$id.'" class="tab-content '. $active .'">';
    if($tab['galeria']):
      echo '<div class="swiper-container galeria-wraper galeria-'.$id.'" >';
